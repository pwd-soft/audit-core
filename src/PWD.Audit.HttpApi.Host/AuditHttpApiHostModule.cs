...
namespace PWD.Audit
{
    ...
    public class AuditHttpApiHostModule : AbpModule
    {
        ...
        private void ConfigureCache(ServiceConfigurationContext context)
        {
            ...
            Configure<AbpDistributedCacheOptions>(options => { options.KeyPrefix = "Audit:"; });
        }
                ...
                Configure<AbpVirtualFileSystemOptions>(options =>
                {
                    options.FileSets.ReplaceEmbeddedByPhysical<AuditDomainSharedModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $"..{Path.DirectorySeparatorChar}PWD.Audit.Domain.Shared"));
                    options.FileSets.ReplaceEmbeddedByPhysical<AuditDomainModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $"..{Path.DirectorySeparatorChar}PWD.Audit.Domain"));
                    options.FileSets.ReplaceEmbeddedByPhysical<AuditApplicationContractsModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $"..{Path.DirectorySeparatorChar}PWD.Audit.Application.Contracts"));
                    options.FileSets.ReplaceEmbeddedByPhysical<AuditApplicationModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $"..{Path.DirectorySeparatorChar}PWD.Audit.Application"));
                });
            }
        }
            ...
            IWebHostEnvironment hostingEnvironment)
        {
            if (!hostingEnvironment.IsDevelopment())
            {
                ...
                context.Services
                    .AddDataProtection()
                    .PersistKeysToStackExchangeRedis(redis, "Audit-Protection-Keys");
            }
        }
        ...
        public override void OnApplicationInitialization(ApplicationInitializationContext context)
        {
            ...
            {
                options.SwaggerEndpoint("/swagger/v1/swagger.json", "Audit API");

                var configuration = context.GetConfiguration();
                options.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
                options.OAuthClientSecret(configuration["AuthServer:SwaggerClientSecret"]);
                options.OAuthScopes("Audit");
            });
